"""
TFinance dataset loader for graph-based anomaly detection.
Provides a single class 'tfinance_data' to access train/val/test datasets.
"""
...
class tfinance_data:
    """
    TFinance fraud detection dataset in homogeneous graph format.
    ...
    def __init__(self, train_ratio=0.4, val_ratio=0.2, random_state=2, undirected=False, verbose=False):
        """
        Initialize and prepare the TFinance dataset.
    ...
    def _load_dataset(self):
        """Load the TFinance dataset from the binary file."""
        if self.verbose:
            print("Loading TFinance dataset...")

        # Load the graph from the binary file
        graph_list, label_dict = load_graphs('/home/liuy133/git/PIGNN_Water/graphormer2/dataset/tfinance')
        self.graph = graph_list[0]

        # Convert one-hot labels to class indices
        # Original: [1, 0] -> 0 (normal), [0, 1] -> 1 (anomaly)
        self.labels = self.graph.ndata['label'].argmax(1).long()
    ...
    def _create_splits(self):
        ...
        # All nodes are labeled in TFinance
    ...
    def __repr__(self):
        return (f"tfinance_data(nodes={self.graph.num_nodes():,}, "
...
if __name__ == "__main__":
    data = tfinance_data(undirected=True, verbose=True)
