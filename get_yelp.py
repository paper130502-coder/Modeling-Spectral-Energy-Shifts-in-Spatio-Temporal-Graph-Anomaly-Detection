"""
Yelp dataset loader for graph-based anomaly detection.
Provides a single class 'yelp_data' to access train/val/test datasets.
"""
...
class yelp_data:
    """
    Yelp fraud detection dataset in homogeneous or heterogeneous graph format.

    Usage:
        # Load as homogeneous graph (default)
        data = yelp_data(homo=True)

        # Load as heterogeneous graph
        data = yelp_data(homo=False)
    ...
    def __init__(self, train_ratio=0.4, val_ratio=0.2, random_state=2, homo=True, undirected=False, verbose=False):
        """
        Initialize and prepare the Yelp dataset.
    ...
    def _load_dataset(self):
        """Load the Yelp dataset and optionally convert to homogeneous graph."""
        if self.verbose:
        ...
        if self.homo:
            ...
            if self.verbose:
                print(f"Homogeneous graph created: {self.graph.num_nodes():,} nodes, {original_edges:,} edges")

            # Convert to undirected if requested
            if self.undirected:
                # Make graph undirected by ensuring edges exist in both directions
                self.graph = dgl.to_bidirected(self.graph, copy_ndata=True)
                # Calculate unique edges (bidirected creates u->v and v->u for each edge)
                unique_edges = self.graph.num_edges() // 2
                if self.verbose:
                    print(f"Converted to undirected: {self.graph.num_edges():,} edges ({unique_edges:,} unique edges)")

            # Add self-loops
            self.graph = dgl.add_self_loop(self.graph)

            if self.verbose:
                print(f"After adding self-loops: {self.graph.num_edges():,} edges")
        else:
    ...
    def _create_splits(self):
        """Create train/val/test splits from all labeled nodes."""
        # For Yelp, all nodes are labeled (no unlabeled nodes like Amazon)
        labeled_indices = list(range(len(self.labels)))
        labeled_labels = self.labels
    ...
    def __repr__(self):
        return (f"yelp_data(nodes={self.graph.num_nodes():,}, "
...
if __name__ == "__main__":
    data = yelp_data(undirected=True, verbose=True)
